...
public class WsClient
{
    ...
    public static Task Send(string message)
    {
        ...
        return ws.SendAsync(buffer, WebSocketMessageType.Text, true /* is last part of message */, CancellationToken.None);

    }
    public static Task<WebSocketReceiveResult> recieveTest()
    {
        var receiveBody = new byte[100];
        return ws.ReceiveAsync(new ArraySegment<byte>(receiveBody), CancellationToken.None);
